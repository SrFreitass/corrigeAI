...
export function CourseCard({
  ...
  return (
    <div
      className="transition-all cursor-pointer w-[32rem] w-max-[32rem] p-8 flex flex-col items-start gap-4 rounded-bg border-third border-2"
    ...
    >
      ...
      <div className="flex gap-4">
        {tags.map((tag, index) => {
          return (
            <p
              className="text-nowrap text-ellipsis px-4 py-1 max-h-8  bg-third rounded-lg text-white"
              key={index}
            >
              {tag}
            </p>
